...
Native editing roles restore platform shortcuts without exposing clipboard IPC to the renderer. The window-owned context menu uses Electron editing flags for selection and input actions. Browser handoff accepts only HTTP(S) destinations without URL credentials from windows whose top-level document is `dsh-app://app` or `dsh-app://shell`, including preview-frame context actions, retains child-window denial, and blocks external redirects. OS handoff failures show locale-owned recovery advice without the private destination query. [Window interaction tests](../../../../apps/desktop/tests/window-interactions.spec.ts) and [main startup tests](../../../../apps/desktop/tests/main-startup.spec.ts) cover the policy and its actual window wiring; native menu text has owner-local expectations because it is not Session-driven output.
...
Local Windows and macOS installation testing uses an explicit `--unsigned` package invocation with the same build and runtime preparation. It strips certificate and Apple notarization inputs, isolates artifacts in `unsigned-artifacts`, and omits updater configuration and the release completion record. macOS local tests use ad-hoc signatures before runtime inventory generation. Their visible App name matches the release product, while the application ID and artifact names identify the local-test build; they do not claim Developer ID ownership, notarization, or Gatekeeper acceptance on another machine. The regular package command explicitly selects signed mode even when its parent environment requests unsigned mode. This separation permits installation diagnosis without release credentials while preventing local test output from qualifying for release upload.
...
**Use message copy buttons without native editing roles, or permit arbitrary Electron navigation.** Message buttons do not restore text-selection shortcuts or input editing. Unrestricted navigation or protocol handoff would grant rendered links unnecessary desktop access. Native menu roles plus a bounded HTTP(S) browser handoff preserve the sandboxed document and keep workspace previews in the application.
