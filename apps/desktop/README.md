...
Desktop serves login explanations and update dialogs from the bundled `renderer/` directory at `dsh-app://shell/`, independently of Host readiness.
...
On Apple Silicon, configure `apps/desktop/.env.macos` with a distinct local-test `DSH_DESKTOP_APP_ID` and the required mandatory-update policy origin before building without Apple Developer credentials. Packaging reads these settings from that file; an inline environment variable does not override them:

```sh
CI=true pnpm run package:desktop:mac:arm64:unsigned
```
