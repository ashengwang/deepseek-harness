...
Desktop 从打包内的 `renderer/` 目录向 `dsh-app://shell/` 提供登录说明和更新弹窗，不依赖 Host 就绪。
...
在 Apple Silicon 上，先在 `apps/desktop/.env.macos` 中配置独立的本机测试 `DSH_DESKTOP_APP_ID` 和必填的强制更新策略 origin，再构建无需 Apple Developer 凭据的应用。打包从该文件读取这些设置，命令行环境变量不会覆盖它们：
