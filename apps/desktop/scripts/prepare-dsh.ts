...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      ...
      if (process.env.DSH_DESKTOP_UNSIGNED === '1') await signMacOSLocalRuntime(join(RUNTIME_ROOT, 'primary-runtime'), appId)
      else await signMacOSRuntime(join(RUNTIME_ROOT, 'primary-runtime'), appId, resolveMacOSSigningEnvironment(process.env))
    }
