...
import { signMacOSLocalRuntime, signMacOSRuntime } from '../scripts/macos-runtime.ts'
import {
...
vi.mock('../scripts/verify-macos-signature.mjs', () => ({
  signMacOSRuntimeCode: vi.fn(), verifyMacOSRuntimeCode: vi.fn(),
  signMacOSLocalRuntimeCode: vi.fn(), verifyMacOSLocalRuntimeCode: vi.fn(),
}))
const roots: string[] = []
...
it('seals local native files without calling release signers', async () => {
  const path = root()
  writeFileSync(join(path, 'addon.node'), Buffer.from('cffaedfe00000000', 'hex'))
  writeFileSync(join(path, 'source.js'), 'export {}')
  await expect(signMacOSLocalRuntime(path, 'com.example.app.local-test')).resolves.toBe(1)
  expect(signMacOSLocalRuntimeCode).toHaveBeenCalledWith(join(path, 'addon.node'), expect.any(String))
  expect(verifyMacOSLocalRuntimeCode).toHaveBeenCalledWith(join(path, 'addon.node'))
