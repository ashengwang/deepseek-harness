...
With focus inside rendered Markdown, Command-P on macOS or Control-P on Windows and Linux loads any remaining text pages and opens the system print preview for the complete rendered document alone. A hidden same-document print copy applies page margins and print-safe wrapping without changing the visible reader. Synthetic key events do not start printing; a page-read failure stops automatic completion and leaves the existing retry control available. The print copy is removed after the browser reports `afterprint`.

Rendered Markdown uses a centred 960px maximum reading column with a 21px body line height and compact section spacing. Tables use denser cells with theme-aware header and body fills; tables with many columns retain their horizontal scroll rather than compressing values below a readable width. These document-only styles also apply to the print copy and do not change Markdown inside chat messages.

HTML fills the body edge to edge in a Blob iframe with exactly `sandbox="allow-scripts"`, without `allow-same-origin`; scripts cannot access the parent application's origin or file reader. With focus inside this frame, Command-P on macOS or Control-P on Windows and Linux opens the system print preview for the HTML document alone. A capability token authenticates the frame's request; only the resulting short-lived print copy adds `allow-modals`, and the copy unmounts after `afterprint`. The renderer loads directly declared relative `.js` classic scripts and `.css` stylesheets through its ordinary inject callback to `remote.workspaceFiles.readRelated`, with fixed safety limits of 4 MiB per asset, 32 MiB total, and 64 distinct assets. Host code resolves the related path; `rpc.ts` decodes the returned bytes. Inside the renderer, base64 is used only to embed the iframe bootstrap payload in script text. A `<base href>` leaves dependency resolution to the browser, as do HTTPS resources. Local module imports, CSS `url()`/`@import`, and dynamic `fetch` do not use Host file access. Read failures, invalid UTF-8, or exceeded limits fail the preview rather than publishing a partial asset package. Replacing or unmounting the document releases both Blob URLs.
...
- **Printing is limited to HTML and Markdown.** Command-P or Control-P prints a focused HTML or Markdown preview; code, PDF, images, and plain text do not yet project a print-only document.
